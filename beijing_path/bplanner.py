...
def judge_line(start_node, end_node, G, ends):
    ...
    def explore_dfs(starts, ends, line):
        ...
        for node in G.neighbors(starts):
            line.append(node)
            if len(line)>=3:
                if criter3(line) and criter4(line) and criter5(line):
    ...
    explore_dfs(node_start_tmp, node_end_tmp, [node_start_tmp])
    return lines
...
print(tml)
print("ratio of getting line:", overlap_rate(tml, line_info, key))
